...
# - - - Conference objects - - - - - - - - - - - - - - - - -
	...
	@endpoints.method(message_types.VoidMessage, ConferenceForms,
			path='getEmptyConferences',
			http_method='POST', name='getEmptyConferences')
	def getEmptyConferences(self, request):
		"""Return empty conferences."""
		# create ancestor query for all key matches for this user
		conf = Conference.query()
		attendees = Conference.query(Conference.maxAttendees)
		seats =  Conference.query(Conference.seatsAvailable)
...
# - - - Registration - - - - - - - - - - - - - - - - - - - -
	...
	def _createSessionObject(self, request):
		...
		if conf.organizerUserId != user_id:
			raise endpoints.BadRequestException("Only Organizer can Create new Session.")
		...
		sessions = Session.query(Session.speaker == request.speaker).count()
		if sessions >= 2:
			taskqueue.add(params={'speaker': request.speaker, 'session_name': request.name}, url='/tasks/set_featured_speaker')
		return SessionForm()
	...
	def getConferenceSessionsByType(self, request):
		"""Return requested type of sessions by conference (by websafeConferenceKey)."""
		# get Session object from request; bail if not found
		conf = ndb.Key(urlsafe=request.websafeConferenceKey)
		if not conf:
			raise endpoints.NotFoundException(
	...
	@endpoints.method(WISH_GET_REQUEST, BooleanMessage,
				path='session/{websafeSessionKey}',
				http_method='DELETE', name='DeleteSessionToWishlist')
	...
	def _sessionWishList(self, request, reg=True):
		...
		if reg:
			...
			if wsck in prof.sessionKeysToAttend:
				raise ConflictException(
					"You've already added this session to your wishlist")
	...
	def getSessionsInWishlist(self, request):
		"""Get wishlist of sessions that user wants to attend."""
		prof = self._getProfileFromUser() # get user Profile
...
# - - - Announcements - - - - - - - - - - - - - - - - - - - -
	...
	def _cacheSpeaker(speaker, wsck):
		"""Check if speaker exists and assign to memcache"""
		conf = ndb.Key(urlsafe=wsck).get()
		if not conf:
			raise endpoints.NotFoundException(
				'No conference found with key: %s' % wsck)
		sessions = Session.query(Session.conference == conf.key)
		sessions = sessions.filter(Session.speaker == speaker)
		if sessions:
			memcache.set(MEMCACHE_FEATURED_SPEAKER, speaker)
		return speaker       
		return request	
